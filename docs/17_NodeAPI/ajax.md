...
Avant de continuer à parler des API, il faut introduire la notion d'AJAX et plus particulièrement de la fonction `fetch()`.

Commençons avec un exemple simple comme la soumission d'un formulaire.
...
- utiliser des API simples,

- par exemple pour appeller un LLM d'un site tier comme Hugging Face.


Vous n’avez pas besoin de connaître tous les détails techniques d’AJAX.  
Vous devez surtout comprendre que cela nous permet de **connecter JavaScript à un serveur**, et d’utiliser les **données dynamiques** sans recharger la page. Le serveur auquel `fetch()` permet de se connecter peut être votre propre backend mais également un autre serveur, comme par exemple le serveur d'Hugging Face.


## 4.2 Introduction à `fetch()`

Pour faire une requête AJAX avec JavaScript moderne, on utilise généralement la fonction **`fetch()`**.  
Elle permet de **demander des données à un serveur**, et de **récupérer une réponse** sans recharger la page.
